...
if(isset($_GET["id"])){
  $id=$_GET["id"];
  $courses=$db->prepare('SELECT * FROM show_course WHERE id= :id');
...
if(isset($_POST["edit"]) && ($_POST["name"]) && ($_POST["Professor_name"]) && ($date=$_POST["date"] && ( $time=$_POST["time"]))){
  ...
  $time=$_POST["time"];
  $unit=$_POST["unit"];
  $courses=$db->prepare('UPDATE show_course SET name=:name,Professor_name=:Pname,date=:date,time=:time,unit=:unit WHERE id =:id ');
