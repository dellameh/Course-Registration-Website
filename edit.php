...
if(isset($_GET["id"])){
  $id=$_GET["id"];
  $courses=$pdo->prepare('SELECT * FROM show_course WHERE id= :id');
  $courses-> execute(["id"=>$id]);
  $course=$courses->fetch();

}
if(isset($_POST["edit"]) && ($_POST["name"]) && ($_POST["professor_name"]) && ($date=$_POST["date"] && ( $time=$_POST["time"]))){
  $name=$_POST["name"];
  $Pname=$_POST["professor_name"];
  $date=$_POST["date"];
  $time=$_POST["time"];
  $unit=$_POST["unit"];
  $courses=$pdo->prepare('UPDATE show_course SET name=:name,professor_name=:Pname,date=:date,time=:time,unit=:unit WHERE id =:id ');
  $courses->execute(["name"=>$name,"Pname"=>$Pname,"date"=>$date,"time"=>$time,"id"=>$id,"unit"=>$unit]);
            ...
            <ul class="nav flex-column pe-3">
              ...
              <li class="nav-item">
                ...
                <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 mb-5">
                    ...
                    >
                        <h1 class="fs-3 fw-bold">ویرایش  درس </h1>
                    </div>
